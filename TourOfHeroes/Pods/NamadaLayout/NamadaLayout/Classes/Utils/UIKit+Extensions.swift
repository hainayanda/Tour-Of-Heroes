...
extension UIButton {
    ...
    struct AssociatedKey {
        static var clickedClosure: String = "Namada_Clicked_Closure"
    }
    
    @objc class ClickedClosure: NSObject {
    ...
    public func didClicked(then: @escaping (UIButton) -> Void) {
        let clickedClosure = ClickedClosure(then)
        addTarget(clickedClosure, action: #selector(ClickedClosure.invoke(with:)), for: .touchUpInside)
        objc_setAssociatedObject(self, &AssociatedKey.clickedClosure, clickedClosure, .OBJC_ASSOCIATION_RETAIN)
    }
}
